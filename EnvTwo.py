...
class EnvTwo(gym.Env):
    ...
    prop_empty = 0.0, alphabet_real = False, allow_backwards = True):
        ...
        if alphabet_real:
            ...
            if prop_empty > 0:
                ...
                for char in self.alphabet:
                    self.alphabet[char] = (self.alphabet[char] / total_letters) * target_sum
        ...
        self._target_start_pos = [-1, -1]  # Starting position of target in grid [row, col]
        self._target_end_pos = [-1, -1]  # Ending position of target in grid [row, col]
        ...
        self.observation_space = spaces.Dict({
            "agent_location": spaces.Box(low=0, high=size-1, shape=(2,), dtype=np.int32),  # [row, col]
            "current_character": spaces.Discrete(29),  # 0 is error, 1-26 is a-z, 27 is _, 28 is #
    ...
    def _generate_grid(self):
        '''
        Generating a 2D grid based on the probabilities, size, target_string, and target_size.
        This uses np.random.choice, a great function that can basically do the request function automatically.
        Uses the environment's seeded random number generators for reproducibility.
        '''
        ...
        if self.target_size in COMMON_WORDS_BY_LENGTH:
            ...
            # Fallback: choose a random substring from the grid if target_size not in common words
            ...
            return grid, target_string, [row, col], [row, col + self.target_size - 1], 'horizontal', 'forward'
    ...
    def _char_to_num(self, char):
        '''
        This is solely for the observation space, printing out 0 if some error has occurred,
        1-26 for all the letters a through z, and 27 for if the spot is "_" or blank.
        '''
        if char == '_':
            return 27
        elif char == '#':
    ...
    def _get_obs(self):
        '''
        Get current observation including agent location, current character, and target string
        '''
    ...
    def reset(self, seed: Optional[int] = None, options: Optional[dict] = None):
        ...
        Generates new random 2D environment, places agent at center position,
        ...
        (self._grid, self._target_string, self._target_start_pos, 
         self._target_end_pos, self._target_direction, self._target_orientation) = self._generate_grid()
    ...
    def render(self, mode='human'):
        ...
        if mode == 'human':
            ...
            print(f"Target: '{self._target_string}' ({self._target_direction}, {self._target_orientation})")
            print(f"Target location: ({self._target_start_pos[0]}, {self._target_start_pos[1]}) to ({self._target_end_pos[0]}, {self._target_end_pos[1]})")
